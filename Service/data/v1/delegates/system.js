...
module.exports = function () {
    ...
    function create(app, req) {
        ...
        try {
            save(app, req.body, changePerson, true)
                .then(function (data) {
                    teamSystem.ShareNewSystem(app, req, data)
                        .then(function () {
    ...
    function membership(app, req) {
        ...
        var model = require('./../models/' + objectType)(app.db).Model();
        var sql = [
            'select  a.* from system a  inner join teamsystem b on a.id = b.systemid  inner join teamdetail c on c.teamid = b.teamid where c.personid = \'{0}\''.format(person)
        ].join();
        return base.GetEntityModels(app, model, sql);
    }
